        ...
        try:
            ...
            if new_refresh_token and new_refresh_token != refresh_token:
                ...
                if self.save_refresh_token(new_refresh_token):
                    ...
                    logger.warning("Failed to save new refresh token - please update manually")
...
def fortnox_rest_api_source(
    ...
    This source fetches various data from Fortnox API.
    ...
    # Configure REST API client with simple single-page fetch for most endpoints
    config: RESTAPIConfig = {
        ...
        "resources": [
            # Endpoints with no data - kept for completeness
            {
                "name": "invoices",
            ...
            },
            {
                ...
                "primary_key": "GivenNumber",
                "endpoint": {
                    "path": "/3/supplierinvoices",
                    "params": {"limit": 500},
                    "data_selector": "SupplierInvoices",
                },
            },
            {
                "name": "vouchers",
                "primary_key": "VoucherNumber",
                "endpoint": {
                    "path": "/3/vouchers",
                    "params": {"limit": 500},
    ...
    logger.info("Configured 9 Fortnox resources")
...
def run_pipeline():
    ...
    try:
        ...
        logger.info(f"  Total packages: {len(load_info.load_packages)}")
        for package in load_info.load_packages:
            logger.info(f"  Package {package.load_id}: {package.state}")
            logger.info(f"    Jobs completed: {len(package.jobs['completed_jobs']) if isinstance(package.jobs, dict) else 'N/A'}")
